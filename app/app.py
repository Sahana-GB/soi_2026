...
TOMTOM_API_KEY = st.secrets.get("TOMTOM_API_KEY", "YOUR_TOMTOM_API_KEY_HERE")
...
def run_b(origin, destination, requests_list, capacity, distance_matrix):
...
def fetch_osrm_matrix_segment_specific(coordinates, names, traffic_factors):
        ...
        if response.status_code == 200:
            ...
            for i, row_name in enumerate(names):
                ...
                for j, col_name in enumerate(names):
                    segment_traffic = (traffic_factors.get(row_name, 1.0) + traffic_factors.get(col_name, 1.0)) / 2.0
                    matrix[row_name][col_name] = round((raw_matrix[i][j] / 1000.0) * segment_traffic, 3)
            return matrix
    except Exception: pass
    ...
    for i, row_name in enumerate(names):
        ...
        for j, col_name in enumerate(names):
            segment_traffic = (traffic_factors.get(row_name, 1.0) + traffic_factors.get(col_name, 1.0)) / 2.0
            matrix[row_name][col_name] = round(haversine_fallback(coordinates[i], coordinates[j]) * segment_traffic, 3)
    return matrix
...
if st.session_state.app_page == "front":
    ...
    if st.session_state.app_mode == "real_time":
                    ...
                    if src_coords and dest_coords:
                        ...
                        if matrix_traffic:
                            ...
                            if result and len(result.get("route", [])) > 0:
                                ...
                                traffic_adjusted_dist = result.get('distance', normal_distance)
                                m_col1, m_col2, m_col3, m_col4 = st.columns(4)
                                m_col1.metric("Effective Satisfaction Score", f"{max(0.0, result.get('score', 0.0)):.3f}")
                                m_col2.metric("Traffic-Adjusted Distance", f"{traffic_adjusted_dist:.2f} km")
                            ...
                            st.error("No valid matrix paths parsed.")
                    else:
                ...
                with st.spinner("Analyzing pooling tracks and traffic..."):
                    ...
                    if src_coords and dest_coords:
                        ...
                        for r in requests_data:
                            ...
                            display_mapping[p_id] = f"{r['name']} Pickup"
                            display_mapping[d_id] = f"{r['name']} Drop"
                        ...
                        if not geocode_failed:
                            ...
                            if matrix_traffic:
                                ...
                                if result and result.get("status") == "Accepted":
                                    ...
                                    m_col1.metric("Traffic-Adjusted Distance", f"{traffic_adjusted_dist:.2f} km")
                                    ...
                                    if "exceeded" in result.get('message', ''):
                                        st.error("Exceeded the capacity limits.")
                                    else:
                                        st.error(f"Rejected: {result.get('message')}")
                        else: 
                            st.error("Some intermediate route location lookups timed out.")
        ...
        try:
            ...
            st.error("Dependencies 'part_a' or 'part_b' modules containing the calculation backends are missing.")
            st.stop()
            ...
            if st.session_state.step_ag == 1:
                ...
                for i in range(n):
                    ...
                    for j in range(n):
                        ...
                        if i == j: 
                            cols[j].text_input(f"{u}→{v}", "0", disabled=True, key=f"g_ma_{i}_{j}")
                        elif j > i:
                            ...
                            cols[j].text_input(f"{u}→{v}", str(raw_matrix[v][u]), disabled=True, key=f"g_ma_{i}_{j}")
