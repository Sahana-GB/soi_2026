        ...
        if response.status_code == 200:
            ...
            for i, row_name in enumerate(names):
                ...
                for j, col_name in enumerate(names):
                    # Blend the traffic factors of both the origin and destination points for this segment
                    ...
                    if src_coords and dest_coords:
                        ...
                        if matrix_traffic:
                            ...
                            if result and len(result.get("route", [])) > 0:
                                ...
                                # Calculate traffic metrics dynamically
                                traffic_dist = result.get('distance', normal_distance)
                                base_duration_mins = (normal_distance / 40.0) * 60.0
                                traffic_multiplier = max(1.0, traffic_dist / max(0.001, normal_distance))
